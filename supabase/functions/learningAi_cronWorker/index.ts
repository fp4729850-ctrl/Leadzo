...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Fetch campaigns directly from database
    const { data: launchedCampaigns } = await supabase.from('launched_campaigns').select('*');
    const { data: adCampaigns } = await supabase.from('ad_campaigns').select('*');

    if (!launchedCampaigns || launchedCampaigns.length === 0) {
        return new Response(JSON.stringify({ message: "No campaigns to analyze" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    // 2. Calculate some basic aggregates
    let totalSpend = 0;
    let totalRevenue = 0;
    let totalClicks = 0;
    ...
    // 3. Ask Gemini to extract learnings
...
Respond ONLY with a JSON object exactly matching this structure:
{
  ...
  "topCreatives": [
    ...
    const userPrompt = `Launched Campaigns: ${JSON.stringify(launchedCampaigns)}\n\nAd Campaigns Metrics: ${JSON.stringify(adCampaigns)}`;
