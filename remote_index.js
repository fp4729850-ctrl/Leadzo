...
app.post('/api/connect', async (req, res) => {
  ...
  client.on('message', async (msg) => {
    try {
      ...
      let contactName = msg.from.replace('@c.us', '').replace(/[^0-9]/g, '');
      try {
        const contact = await msg.getContact();
        // Use pushname (WhatsApp display name) or saved name
        contactName = contact.pushname || contact.name || contact.number || `+${msg.from.split('@')[0]}`;
      } catch(e) {
        contactName = `+${msg.from.split('@')[0]}`;
      }
      
      const fromNumber = msg.from; // Keep the exact ID (with @c.us or @lid)
  ...
  for (let num of numbers) {
    try {
      ...
      if (!cleanNum.includes('@')) {
        cleanNum = cleanNum.replace(/[^0-9]/g, '') + '@c.us';
      }
