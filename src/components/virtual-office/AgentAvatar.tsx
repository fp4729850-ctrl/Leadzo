...
export function AgentAvatar({ name, role, position, statusMessage, isMoving }: AgentAvatarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Full Body Character SVG */}
      <div className={`relative group flex flex-col items-center ${isMoving ? 'animate-bounce' : ''}`}>
        <CharacterSvg isMoving={isMoving} role={role} />
        
        {/* Name Tag (placed right below the character) */}
        <div className="mt-1 bg-slate-900/80 backdrop-blur-sm text-white text-[10px] sm:text-[11px] font-bold px-2 py-0.5 rounded shadow-lg border border-slate-700/50 pointer-events-none">
          {name}
        </div>
      </div>
    </div>
