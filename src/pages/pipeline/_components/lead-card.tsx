...
const INTENT_BADGE: Record<string, string> = {
  BUY: "bg-chart-3/10 text-chart-3 border-chart-3/20",
...
export default function LeadCard({ lead, onSelect }: Props) {
  ...
  return (
    <div
      ...
      className="group relative bg-card border border-border rounded-xl p-4 cursor-pointer hover:border-primary/40 hover:shadow-md transition-all duration-200"
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="size-8 rounded-lg bg-muted flex items-center justify-center shrink-0 text-sm">
        ...
        {lead.isScam ? (
          ...
          <Shield size={15} className="text-chart-3/50 shrink-0 mt-0.5" />
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 mb-3">
        <span
          className={cn(
            ...
            INTENT_BADGE[lead.intent] ?? INTENT_BADGE.NONE
          )}
        >
          {lead.intent}
        </span>
        ...
        {lead.isUrgent && (
          <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border bg-chart-4/10 text-chart-4 border-chart-4/20">
            <Zap size={9} />
            Urgent
          </span>
        )}
      ...
      {lead.lastMessage && (
        <p className="text-xs text-muted-foreground truncate italic border-t border-border pt-2.5 mt-2.5">
          &ldquo;{lead.lastMessage}&rdquo;
        </p>
      )}

      <div
        className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity"
          ...
          <DropdownMenuContent align="start" className="w-40">
            ...
            {STATUS_OPTIONS.filter((s) => s.value !== lead.status).map((s) => (
              <DropdownMenuItem
                ...
                className={cn("text-xs cursor-pointer", s.className)}
              >
                {s.label}
              </DropdownMenuItem>
            ))}
